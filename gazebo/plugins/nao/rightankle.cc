...
#include "rightankle.h"

namespace gazebo {
    GZ_REGISTER_MODEL_PLUGIN(RightAnkle)

    RightAnkle::RightAnkle () {
        pthread_mutex_init(&this->mutex_rightankleencoders, NULL);
        pthread_mutex_init(&this->mutex_rightanklemotors, NULL);
        this->cycle = 50;
        this->cfgfile_rightankle = std::string("--Ice.Config=rightankle.cfg");
        //this->modelPitch = std::string("joint_poserightankle_pitch");
        this->modelRoll = std::string("joint_poserightankle_roll");

        std::cout << "********** Constructor RightAnkle **********" << std::endl;
    }

    void RightAnkle::Load ( physics::ModelPtr _model, sdf::ElementPtr _sdf ) {
        ...
        //std::string elemPitch = std::string(_sdf->GetElement(this->modelPitch)->Get<std::string>());
        std::string elemRoll = std::string(_sdf->GetElement(this->modelRoll)->Get<std::string>());
...
*/
       ...
       // this->minPitch = (float) this->rightankle.joint_pitch->GetLowerLimit(0).Radian();
        ...
        if (_sdf->HasElement("torque"))
            this->stiffness = _sdf->GetElement("torque")->Get<double>();
        else {
	...
	/*math::Pose* posicion = new math::Pose(10, 10, 0, 0, 0, 0);
	_model->SetWorldPose(*posicion, true, false);
	std::cout << "POSEEEEEEE " << _model->GetWorldPose() << std::endl;*/

        // Load OnUpdate method
        this->updateConnection = event::Events::ConnectWorldUpdateBegin(
                                    boost::bind(&RightAnkle::OnUpdate, this));
    }

    void RightAnkle::Init () {
    ...
    void RightAnkle::OnUpdate () {
        ...
        //          ----------MOTORS----------
        //this->rightankle.joint_pitch->SetParam("fmax",0, this->stiffness);
        this->rightankle.joint_roll->SetParam("fmax",0, this->stiffness);
	...
	//this->rightankle.joint_pitch->SetPosition(0, 0);
        //this->rightankle.joint_roll->SetPosition(0, 0);
    ...
    public:
        ...
        Pose3DEncodersRA ( gazebo::RightAnkle* pose ) : pose3DEncodersData ( new jderobot::Pose3DEncodersData() ) {
            this->pose = pose;
        }
    ...
    public:
        ...
        Pose3DMotorsRA (gazebo::RightAnkle* pose) : pose3DMotorsData ( new jderobot::Pose3DMotorsData() ) {
            this->pose = pose;
        }
    ...
    void* thread_RightAnkleICE ( void* v ) {
        ...
        gazebo::RightAnkle* rightankle = (gazebo::RightAnkle*)v;
        ...
        try {
            ...
            std::string EndpointsEncoders = prop->getProperty("RightAnkleEncoders.Endpoints");
            std::cout << "RightAnkleEncoders Endpoints > " << EndpointsEncoders << std::endl;
            std::string EndpointsMotors = prop->getProperty("RightAnkleMotors.Endpoints");
            std::cout << "RightAnkleMotors Endpoints > " << EndpointsMotors << std::endl;
