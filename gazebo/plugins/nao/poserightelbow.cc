...
namespace gazebo {
    ...
    void PoseRightElbow::Load ( physics::ModelPtr _model, sdf::ElementPtr _sdf ) {
        ...
        std::string elemPan = std::string(_sdf->GetElement(this->modelYaw)->Get<std::string>());
...
//        this->rightelbow.link_roll = this->rightelbow.joint_roll->GetParent();
    ...
    void PoseRightElbow::OnUpdate () {
        ...
        //          ----------MOTORS----------
        this->rightelbow.joint_yaw->SetParam("fmax",0, this->stiffness);
	...
	this->error_roll = this->rightelbow.motorsdata.roll - this->rightelbow.encoders.roll;
        ...
        double yawSpeed =  20*(this->rightelbow.motorsdata.pan - this->rightelbow.encoders.pan) + 0.1*(this->error_yaw - this->error_yaw_ant);
        //if ((std::abs(yawSpeed) < 0.1) && (std::abs(yawSpeed) > 0.001))
        //    yawSpeed = 0.1;
        
        double rollSpeed =  20*(this->rightelbow.motorsdata.roll - this->rightelbow.encoders.roll) + 0.1*(this->error_roll - this->error_roll_ant);
