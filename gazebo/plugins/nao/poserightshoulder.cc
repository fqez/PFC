...
*/
	...
	this->rightshoulder.joint_pitch->SetParam("fmax",0, this->stiffness);
        ...
        double pitchSpeed =  20*(this->rightshoulder.motorsdata.tilt - this->rightshoulder.encoders.tilt) + 0.1*(this->error_pitch - this->error_pitch_ant);
        //if ((std::abs(pitchSpeed) < 0.1) && (std::abs(pitchSpeed) > 0.001))
        //    pitchSpeed = 0.1;
        
        double rollSpeed =  20*(this->rightshoulder.motorsdata.roll - this->rightshoulder.encoders.roll) + 0.1*(this->error_pitch - this->error_pitch_ant);
